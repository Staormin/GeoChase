...
describe('usePointDrawing', () => {
  ...
  describe('drawPoint', () => {
    ...
    it('should use fallback zoom of 0 when getZoom returns undefined', () => {
      mockView.getZoom.mockReturnValue(undefined);

      const createElementOriginal = document.createElement;
      const createElementSpy = vi
        .spyOn(document, 'createElement')
        .mockImplementation((tag: string) => createElementOriginal.call(document, tag));

      pointDrawing.drawPoint(48.8566, 2.3522, 'Test Point');

      // With zoom 0 (< 12), opacity should be 0
      const labelElement = createElementSpy.mock.results[0].value;
      expect(labelElement.style.cssText).toContain('opacity: 0');
