...
describe('usePolygonDrawing', () => {
  ...
  describe('redrawPolygonOnMap', () => {
    ...
    it('should handle polygon not found in store', () => {
      // Use a polygon ID that doesn't exist in the store
      // Points are still valid so the polygon will be drawn
      polygonDrawing.redrawPolygonOnMap('non-existent-polygon', testPointIds, '#ff0000');
  ...
  describe('Color conversion helper', () => {
    ...
    it('should handle unknown color format', () => {
      const result = polygonDrawing.drawPolygon(
        ['color-p1', 'color-p2', 'color-p3'],
        'Test',
        'hsl(0, 100%, 50%)'
