...
describe('useViewCapture', () => {
  ...
  describe('View Capture Mode', () => {
    ...
    it('should update map size when capturing starts', async () => {
      viewCapture.setup();
      ...
      // Advance timers and flush promises for async watcher
      vi.advanceTimersByTime(100);
      await nextTick();
  ...
  describe('Screenshot Capture', () => {
    ...
    it('should show Start view toast when captureType is start', async () => {
      viewCapture.setup();
      const addToastSpy = vi.spyOn(uiStore, 'addToast');

      // Mock captureView to prevent it from resetting captureType
      vi.spyOn(uiStore, 'captureView').mockImplementation(() => {});

      // captureType is 'start' by default from beforeEach
      uiStore.viewCaptureState.isCapturing = true;

      await mockMapClickHandler();

      expect(addToastSpy).toHaveBeenCalledWith('Start view captured successfully!', 'success');
    });

    it('should show End view toast when captureType is end', async () => {
      viewCapture.setup();
      const addToastSpy = vi.spyOn(uiStore, 'addToast');

      // Mock captureView to prevent it from resetting captureType
      // This allows us to test the ternary with captureType still set to 'end'
      vi.spyOn(uiStore, 'captureView').mockImplementation(() => {});

      // Set captureType to 'end'
      uiStore.viewCaptureState.isCapturing = true;
