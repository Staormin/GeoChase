...
describe('useNavigation', () => {
  ...
  describe('Line Segment Navigation', () => {
    ...
    it('should handle startNavigation for lineSegment without segment object', () => {
      // Set progress to non-zero to verify it stays unchanged when no segment provided
      navigation.navigationState.value.progress = 0.5;

      navigation.startNavigation('lineSegment', 'line-1', undefined, undefined);

      expect(navigation.navigationState.value.active).toBe(true);
      expect(navigation.navigationState.value.elementType).toBe('lineSegment');
      // Progress should not be reset when no segment is provided
      expect(navigation.navigationState.value.progress).toBe(0.5);
    });
    ...
    it('should handle azimuth mode segments for getSegmentNavigationCoords', () => {
    ...
    it('should calculate segment length for azimuth mode segments', () => {
      const azimuthSegment: LineSegmentElement = {
        id: 'line-azimuth',
        name: 'Azimuth Line',
        center: { lat: 48.8566, lon: 2.3522 },
      ...
      navigation.startNavigation('lineSegment', 'line-azimuth', undefined, azimuthSegment);

      // Navigate forward to trigger calculateSegmentLength with azimuth mode
      navigation.navigateSegmentForward(azimuthSegment, 15);

      // Progress should have changed (segment length calculated via destinationPoint)
      expect(navigation.navigationState.value.progress).toBeGreaterThan(0);
    });

    it('should return 0 length for segments without endpoint', () => {
      ...
      navigation.startNavigation('lineSegment', 'line-no-endpoint', undefined, noEndpointSegment);

      // Navigate forward with a segment that has no endpoint
      // calculateSegmentLength returns 0, causing division by zero → Infinity
      navigation.navigateSegmentForward(noEndpointSegment, 15);

      // Progress becomes Infinity due to division by 0, but still reflects at >= 1
      expect(navigation.navigationState.value.progress).toBeLessThanOrEqual(1);
    });
