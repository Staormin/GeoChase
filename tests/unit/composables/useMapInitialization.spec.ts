...
describe('useMapInitialization', () => {
  ...
  describe('Initial Map Setup', () => {
    ...
    it('should not open new project modal when language is not set', async () => {
      // Mock isLanguageSet to return false (language not yet selected)
      vi.mocked(isLanguageSet).mockReturnValueOnce(false);
      ...
      expect(mockMapContainer.initMap).toHaveBeenCalled();
      // Modal should NOT be opened when language is not set
