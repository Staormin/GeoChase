...
<script lang="ts" setup>
  ...
  import { calculateDistance, destinationPoint, mercatorProject, mercatorUnproject } from '@/services/geometry'
  ...
  function calculateMidpoint () {
    ...
    const totalDistance = calculateDistance(segment.center.lat, segment.center.lon, endpoint.lat, endpoint.lon)
  ...
  function submitForm () {
    ...
    const segmentLength = calculateDistance(segment.center.lat, segment.center.lon, endpoint.lat, endpoint.lon)
    ...
    if (form.value.distanceFrom === 'start') {
      ...
      while (iterations < maxIterations) {
        ...
        const testDistance = calculateDistance(segment.center.lat, segment.center.lon, testPoint.lat, testPoint.lon)
      ...
      while (iterations < maxIterations) {
        ...
        const testDistance = calculateDistance(endpoint.lat, endpoint.lon, testPoint.lat, testPoint.lon)
        const tolerance = 0.0001 // 0.1 meter tolerance
